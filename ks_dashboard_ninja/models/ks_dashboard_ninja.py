...
class KsDashboardNinjaBoard(models.Model):
    ...
    def create(self, vals):
        ...
        # comment this part due to the action and menuitem are created from xml
        # code
        ...
        #     })
        if record.ks_dashboard_default_template and \
                record.ks_dashboard_default_template.ks_item_count:
            record.update_gridstack(record.ks_dashboard_default_template.ks_gridstack_config)
        return record
    ...
    def ks_fetch_item_data(self, rec):
        ...
        item = {
            ...
            'ks_gauge_next_item': rec.ks_gauge_next_item.id,
            'ks_show_in_dashboard': rec.ks_show_in_dashboard,
            'ks_dashboard_ninja_group_id': rec.ks_dashboard_ninja_group_id.id,
            'ks_is_first_layer': rec.ks_is_first_layer,
            'ks_empty_chart_msg': rec.ks_empty_chart_msg,
            'ks_show_center_text': rec.ks_show_center_text,
        }
        return item
    ...
    def update_gridstack(self, ks_gridstack):
        """ Update the gridstack based on the template
        """
        for record in self:
            ks_gridstack_config = {}
            template_data = json.loads(ks_gridstack)
            for item_data in template_data:
                dashboard_item = self.env.ref(item_data['item_id'])
                dashboard_item.ks_dashboard_ninja_board_id = record.id
                ks_gridstack_config[dashboard_item.id] = item_data['data']
            record.ks_gridstack_config = json.dumps(ks_gridstack_config)


class KsDashboardItemGroup(models.Model):
    _name = 'ks_dashboard_ninja.group'
    _description = 'Dashboard Ninja Items Group'
    ...
    ks_dashboard_items_ids = fields.One2many(
        ...
        string='Group Items',
        domain="[('ks_dashboard_ninja_board_id','=',ks_dashboard_board_id), "
               "('ks_is_first_layer', '=', False)]",
        required=True)

    @api.model
    def ks_get_grouped_chart(self, group_id):
        """ This function will return the grouped chart ids given group_id.
        """
        item_group = self.browse(group_id)
        item_dict = {item.ks_chart_selection_string: item.id
                     for item in item_group.ks_dashboard_items_ids}
        return item_dict
...
class KsDashboardNinjaTemplate(models.Model):
    ...
    ks_board_ids = fields.One2many('ks_dashboard_ninja.board',
                                   'ks_dashboard_default_template',
                                   string='Boards')

    def write(self, values):
        """ Update the board gridstack config if the template has been updated
        """
        res = super(KsDashboardNinjaTemplate, self).write(values)
        for rec in self:
            # run function update_gridstack for all related board
            for board in rec.ks_board_ids:
                if values.get('ks_gridstack_config'):
                    board.update_gridstack(values.get('ks_gridstack_config'))
        return res
