...
//            css_grid = $('.o_rtl').length>0 ?
            ...
            this.ksChartColorOptions = ['default', 'cool', 'warm', 'neon', 'scheme_four', 'scheme_five', 'scheme_six', 'scheme_seven', 'scheme_eight'];
...
//            self.ks_fetch_data();
        ...
        events: {
            ...
            'click .ks_gauge_item_click': '_onKsGaugeItemClick',
            ...
            'click .group_by_radio': 'ksOnGroupBy',
            'click .ks_chart_filter': 'ksOnFilterBy',
            'click .ks_dashboard_back_button': 'ksOnBackButton',
        },
        ...
        ks_set_default_chart_view: function() {
            ...
            Chart.plugins.register({
                afterDraw: function(chart) {
                    // print the desired empty chart message instead of 'no data available'
                    var msg = chart.config.options['noDataAvailMsg'] || 'No data available';
...
//                        ctx.fillText(msg, width / 2, height / 2);
                        wrapText(ctx, msg, width / 2, height / 2, width, 20);
                        ctx.restore();
                    } else if (chart.config.type == 'tsgauge') {
                        var empty = true;
                        var gaugeLimits = chart.config.data.datasets[0].gaugeLimits;
                        for(var i=0;i < gaugeLimits.length; i++){
                            if(gaugeLimits[i] !== 0) {
                                empty = false;
                                break;
                            }
...
//                            ctx.fillText(msg, width / 2, height / 2);
        ...
        labelAbbreviation: function(label, characterLimit) {
            var result = label;
            if(result.length > characterLimit){
                result = result.substring(0, characterLimit) + '...';
            }
            return result;
        },
        ...
        ksRenderDashboardItems: function(items) {
            ...
            for (var i = 0; i < items.length; i++) {
                // only render the chart with show_in_dashoboard = True
                if (self.grid && items[i].ks_show_in_dashboard) {
        ...
        _ksRenderDashboardTile: function(tile) {
            ...
            var data_count = self.ksNumFormatter(tile.ks_record_count, 4);
        ...
        _renderChart: function($ks_gridstack_container, item) {
            ...
            // load group by button
            // always clear the group by button
            $ks_gridstack_container.find('.ks_group_by').empty();
            if (item.ks_dashboard_ninja_group_id) {
                self._rpc({
                    model: 'ks_dashboard_ninja.group',
                    method: 'ks_get_grouped_chart',
                    args: [item.ks_dashboard_ninja_group_id]
                }).then(function(result) {
                    // create unique name for each radio group using chart_id
                    var radio_name = "chart_group_by_" + chart_id;
                    // the chart might be drilled down from gauge chart, hence
                    // need to get the correct chart_id
                    var currentChart = $ks_gridstack_container.find('.ks_group_by_clause')[0].getAttribute('data-chart-id');
                    if (currentChart) {
                        radio_name = "chart_group_by_" + currentChart;
                    }
                    for (var key in result) {
                        // create unique name for each radio button using chart_id
                        var radio_id = key + "_" + chart_id;
                        // create radio button
                        var $ksChartGroupBy = $('<input type="radio" class="group_by_radio" name="'+ radio_name + '" id="' + radio_id  + '"data-chart-id=' + result[key] + ' value="' + result[key] + '"/><label for="' + radio_id + '">' + key + '</label>');
                        // append the radio button to the div group_by
                        $ks_gridstack_container.find('.ks_group_by').append($ksChartGroupBy);
                    }
                    // mark current chart radio as true
                    // mark first element as true if no current chart is found
                    if (currentChart) {
                        $("input[name='"+ radio_name +"'][value='" + currentChart +"']").prop('checked', true);
                    } else {
                        var firstElement = $ks_gridstack_container.find('.ks_group_by')[0].firstElementChild;
                        firstElement.checked = true;
                        // set first element id as data-chart-id of group_by division
                        $ks_gridstack_container.find('.ks_group_by_clause')[0].setAttribute('data-chart-id', firstElement.getAttribute('data-chart-id'));
                    }
                });
            }
            ...
            // load filter buttons
            self._rpc({
                model: 'ks_dashboard_ninja.item_filter',
                method: 'ks_get_chart_filters',
                args: [item.id]
            }).then(function(result) {
                var reload = false;
                // always clear the filter division
                $ks_gridstack_container.find('.ks_where_clause').empty();
                // get the filter applied
                var filter_applied = self.ks_dashboard_data.ks_item_data[item.id]['filter_applied'];
                for (var filter_group in result) {
                    // create checkbox group
                    var $ksChartFilterGroup = $('<div class="ks_where" id=' + filter_group + '" data-chart-id=' + item.id + '/>');
                    // create checkbox button
                    for (var filter in result[filter_group]){
                        // get filter attribute
                        var filter_label = result[filter_group][filter]['name'];
                        var default_filter = result[filter_group][filter]['default'];
                        var mobile_visible = result[filter_group][filter]['mobile_visible'];
                        // create checkbox button
                        var $ksChartFilter = $('<input type="checkbox" class="ks_chart_filter" id="' + filter + '" data-chart-id='+ item.id + ' name="' + filter + '" value="' + filter_label +'"/><label for="' + filter + '">' + filter_label + '</label>');
                        // append the checkbox button to the group
                        $ksChartFilterGroup.append($ksChartFilter);
                        // if there is no custom filter selected, use default filter
                        if (!filter_applied && default_filter) {
                            reload = true;
                            $ksChartFilter.prop('checked', true);
                        }
                        // apply the filter from previous filter_applied
                        if (filter_applied && filter_applied.includes(filter)) {
                            $ksChartFilter.prop('checked', true);
                        }
                        // hide the filter in mobile view
                        if (!mobile_visible) {
                            $ksChartFilter.addClass('d-none');
                            $ksChartFilter.addClass('d-md-inline');
                        }
                    }
                    $ks_gridstack_container.find('.ks_where_clause').append($ksChartFilterGroup);
                }
                if (reload) {
                    var filterApplied = [];
                    var filters = $('.ks_chart_filter[data-chart-id='+ item.id + ']:checked');
                    for (var index = 0; index < filters.length; index++) {
                        filterApplied.push(filters[index].id);
                    }
                    self.ksApplyFilter(item.id, filterApplied);
                }
            });
            ...
            var divWidth = $ks_gridstack_container.find('.card-body')[0].offsetWidth;
            // best ratio for left right padding
            var padding_ratio = 9.5;
            // code to render gauge chart
            if (chart_type === 'tsgauge'){
                $ksChartContainer.addClass('ks_gauge_item_click');
                var ksMyChart = new Chart($ksChartContainer[0], {
                    ...
                    options: {
                        ...
                        events: [],
                        showMarkers: true,
                        gaugeLayout: chart_data.ks_gauge_layout,
                        arrowAnimation: chart_data.arrow_animation,
                        fontColor: chart_data.font_color,
                        cutoutPercentage: chart_data.thickness,
                        labelUnit: chart_data.ks_field,
                        gaugeDP: chart_data.ks_gauge_dp,
                        arrowColor: chart_data.ks_arrow_color,
                        limitPercent: chart_data.ks_gauge_limits_percent,
                        gaugeColors: chart_data.ks_gauge_colors,
                        showMinMaxOnly: chart_data.ks_show_min_max_only,
                        noDataAvailMsg: chart_data.ks_empty_chart_msg,
                        dateSelection: chart_data.date_selection,
                        layout: {
                            padding: {
                                bottom: 30,
                                left: divWidth / padding_ratio,
                                right: divWidth / padding_ratio,
                            }
                        }
                    }
                });
            } else {
                if (item.ks_is_first_layer) {
                    $ksChartContainer.removeClass('ks_item_click');
                    $ksChartContainer.addClass('ks_gauge_item_click');
                }
                var ksMyChart = new Chart($ksChartContainer[0], {
                    ...
                    options: {
                        ...
                        noDataAvailMsg: chart_data.ks_empty_chart_msg,
                        centerText: item.ks_show_center_text,
                        labelUnit: chart_data.ks_field,
                        animation: {
                            duration: 0,
                        },
        ...
        ksChartColors: function(palette, ksMyChart, ksChartType, ksChartFamily, stack, semi_circle, ks_show_data_value, chart_data, item) {
            ...
            options.tooltips.cornerRadius = 10;
            options.tooltips.titleMarginBottom = 20;
            options.tooltips.titleFontSize = 14;
            options.tooltips.bodyFontSize = 14;
            if (ksChartFamily == "circle") {
                ...
                options.tooltips.callbacks = {
                    title: function(tooltipItem, data) {
                        ...
                        var count = data.datasets[tooltipItem[0].datasetIndex]['count'][tooltipItem[0].index] || 0;
                        var ks_selection = chart_data.ks_selection;
                        if (ks_selection === 'monetary') {
                            var ks_currency_id = chart_data.ks_currency;
                ...
                options.tooltips.titleFontStyle = 'normal';
                options.tooltips.bodyFontStyle = 'bold';
                ...
                if (chartType === "horizontalBar") {
                    ...
                    options.scales.xAxes[0].minBarLength = 20;
                    options.scales.yAxes[0].barPercentage = 1.1;

                    options.scales.yAxes[0].ticks.callback = function(value, index, values) {
                        return self.labelAbbreviation(value, 10);
                    }
                } else {
                    options.scales.xAxes[0].ticks.callback = function(value, index, values) {
                        return self.labelAbbreviation(value, 10);
                    ...
                    options.scales.yAxes[0].minBarLength = 20;
                    options.scales.xAxes[0].barPercentage = 1.1;
                }

                options.tooltips.callbacks = {
                    title: function(tooltipItem){
                        var stacked = this._chart.options.scales.xAxes[0].stacked;
                        if (stacked == true){
                            var sum = 0;
                            for (var i = 0; i < tooltipItem.length; i++){
                                sum += parseInt(tooltipItem[i].value);
                            }
                            var result = this._data.labels[tooltipItem[0].index];
                            console.log(tooltipItem);
                            for (var i = 0; i < tooltipItem.length; i++){
                                var percentage = sum === 0 ? 0 + "%" : (parseInt(tooltipItem[i].value) * 100 / sum).toFixed(2) + "%";
                                result += "\n" + this._data.datasets[tooltipItem[i].datasetIndex]['label'] + ' : ' +  percentage;
                            }
                            return result
                        } else {
                            var count = this._data.datasets[tooltipItem[0].datasetIndex]['count'][tooltipItem[0].index];
        ...
        onChartCanvasClick: function(evt) {
            if (evt.currentTarget.classList.value.includes('ks_gauge_item_click')){
                return;
            }
            var self = this;
            if (evt.currentTarget.classList.value !== 'ks_list_canvas_click') {
                ...
                if (activePoint) {
                    // if the device doesn't have mouse support, then click once
                    // to see the value, click again to drill down.
                    if (!(matchMedia('(pointer:fine)').matches) && (!("pointing" in myChart) || myChart['pointing'] !== activePoint._index)){
                        myChart['pointing'] = activePoint._index;
                        return;
                    } else {
                        var item_data = self.ks_dashboard_data.ks_item_data[item_id];
                            ...
                            if (item_data.max_sequnce != 0 && sequnce < item_data.max_sequnce) {
                                self._rpc({
                                    ...
                                    if (result.ks_chart_data) {
                                        self.ks_dashboard_data.ks_item_data[item_id]['prev_ks_dashboard_item_type'] = self.ks_dashboard_data.ks_item_data[item_id]['ks_dashboard_item_type'];
                                        ...
                                        $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".ks_group_by").empty();
                                        var item_data = self.ks_dashboard_data.ks_item_data[item_id]
                                        self.ksApplyFilter(item_id, item_data['filter_applied']);
                                    } else {
                                ...
                                // check filter applied before 3rd layer view
                                self._rpc({
                                    model: 'ks_dashboard_ninja.item',
                                    method: 'ks_reformat_domain_filter_applied',
                                    args: [domain, item_data['filter_applied']]
...
//                                self._renderChart($(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]), item_data);
                        ...
                        self.ks_dashboard_data.ks_item_data[item_id]['ks_dashboard_item_type'] = self.ks_dashboard_data.ks_item_data[item_id]['prev_ks_dashboard_item_type'];
                        self.ksFetchChartItem(item_id).then(function() {
                            self.ksApplyFilter(item_id, item_data['filter_applied']);
                        });
        ...
        ksOnBackButton: function(e) {
            var self = this;
            // get id of the current chart
            var id = parseInt(e.currentTarget.dataset.itemId);
            // get the item data of the current chart
            var item_data = self.ks_dashboard_data.ks_item_data[id];
            ...
            return self._rpc({
                ...
                method: 'ks_fetch_item',
                args: [
                    [id], self.ks_dashboard_id
                ],
                context: self.getContext(),
            }).then(function(new_item_data) {
                $(self.$el.find(".grid-stack-item[id=" + id + "]").children()[0]).find(".card-body").empty();
                // populate the new chart header
                var new_chart_name = new_item_data[id]['name'];
                $(self.$el.find(".grid-stack-item[id=" + id + "]").children()[0]).find(".ks_chart_heading").html(new_chart_name);
                $(self.$el.find(".grid-stack-item[id=" + id + "]").children()[0]).find(".ks_dashboard_back_button").addClass('d-none');
                $(self.$el.find(".grid-stack-item[id=" + id + "]").children()[0]).find('.ks_group_by_clause')[0].setAttribute('data-chart-id', '');
                self._renderChart($(self.$el.find(".grid-stack-item[id=" + id + "]").children()[0]), item_data);
                // modify the data-gs-id of the first level chart
                var gauge_element = self.$el.find(".grid-stack-item[id=" + id + "]")[0];
                gauge_element.setAttribute('data-gs-id', id);
            }.bind(this));
        },

        ksOnGroupBy: function(e) {
            var self = this;
            // get id of the current chart
            var id = parseInt(e.currentTarget.parentElement.parentElement.getAttribute('data-chart-id'));
            var radio_group = 'chart_group_by_' + id;
            var current_item = self.ks_dashboard_data.ks_item_data[id];
            // get the selected chart id
            var next_id = $("input[name="+ radio_group+"]:checked").val();

            // will show the selected chart.
            var item_data = self.ks_dashboard_data.ks_item_data[next_id];
            ...
            return self._rpc({
                ...
                $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find(".ks_group_by").empty();
                $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find(".ks_where_clause").empty();
                // populate the new chart header
                var new_chart_name = new_item_data[next_id]['name'];
                $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find(".ks_chart_heading").html(new_chart_name);
                self._renderChart($(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]), item_data);
                // modify the data-gs-id of the new chart
                var chart_element = self.$el.find(".grid-stack-item[data-gs-id=" + id + "]")[0];
                var drill_up_button = $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find(".ks_dashboard_item_drill_up")[0];
                var group_by_element = $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find(".ks_group_by_clause")[0];
                drill_up_button.setAttribute('data-item-id', next_id);
                chart_element.setAttribute('data-gs-id', next_id);
                group_by_element.setAttribute('data-chart-id', next_id);
            }.bind(this));

        },

        ksOnFilterBy: function(evt) {
            var self = this;
            var filterApplied = [];
            var item_id = evt.currentTarget.getAttribute('data-chart-id');
            if (item_id in self.ksUpdateDashboard) {
                clearInterval(self.ksUpdateDashboard[item_id]);
                delete self.ksUpdateDashboard[item_id]
            }
            var filters = $('.ks_chart_filter[data-chart-id='+ item_id + ']:checked');
            for (var index = 0; index < filters.length; index++) {
                filterApplied.push(filters[index].id);
            }
            self.ksApplyFilter(item_id, filterApplied);
        },

        ksApplyFilter: function(item_id, filterApplied) {
            var self = this;
            var item_data = self.ks_dashboard_data.ks_item_data[item_id];
            var domain = [];
            if (item_data.sequnce) {
                domain = item_data.domains[item_data.sequnce];
            }
            item_data['filter_applied'] = filterApplied;
            self._rpc({
                model: 'ks_dashboard_ninja.item',
                method: 'ks_fetch_filter_data',
                args: [item_data, filterApplied]
            }).then(function(result) {
                self.ks_dashboard_data.ks_item_data[item_id]['ks_chart_data'] = result;
                $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".card-body").empty();
                self._renderChart($(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]), item_data);
            });
        },
        ...
        _onKsGaugeItemClick: function(e) {
            var self = this;
            // get id of the current gauge chart
            var id = parseInt(e.currentTarget.dataset['chartId']);
            var current_item = self.ks_dashboard_data.ks_item_data[id];
            // get the next level chart of the current gauge chart
            var next_id = current_item.ks_gauge_next_item;

            // will show the next level chart if next_item is declared.
            ...
            if (next_id) {
                ...
                return self._rpc({
                    ...
                    var new_chart_name = new_item_data[next_id]['name'];
                    $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find(".ks_chart_heading").html(new_chart_name);
                    $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find('.ks_group_by_clause')[0].setAttribute('data-chart-id', '');
                    $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find(".ks_dashboard_back_button").removeClass('d-none');
                    self._renderChart($(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]), item_data);
                    // modify the data-gs-id of the next level chart
                    var gauge_element = self.$el.find(".grid-stack-item[data-gs-id=" + id + "]")[0];
                    var drill_up_button = $(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]").children()[0]).find(".ks_dashboard_item_drill_up")[0];
                    drill_up_button.setAttribute('data-item-id', next_id);
                    gauge_element.setAttribute('data-gs-id', next_id);
                    gauge_element.setAttribute('previous-gs-id', id);
                }.bind(this));
            }
        },
        ...
        ksSum: function(count_1, count_2, item_info, field, target_1, $kpi_preview, kpi_data) {
            ...
            item_info['count'] = self.ksNumFormatter(count, 4);
        ...
        renderKpi: function(item, grid) {
            ...
            var item_info = {
                item: item,
                id: field.id,
                count_1: self.ksNumFormatter(kpi_data[0]['record_data'], 4),
                ...
                target: self.ksNumFormatter(target_1, 4),
            ...
            if (!kpi_data[1]) {
                ...
                switch (field.ks_data_comparison) {
                    case "None":
                        ...
                        var count = String(self.ksNumFormatter(count_1, 4)) + "/" + String(self.ksNumFormatter(count_2, 4));
                    ...
                    case "Ratio":
                        ...
                        item_info['count'] = (isNaN(count_1 / gcd) ? 0 : self.ksNumFormatter(count_1 / gcd, 4)) + ":" + (isNaN(count_2 / gcd) ? 0 : self.ksNumFormatter(count_2 / gcd, 4));
