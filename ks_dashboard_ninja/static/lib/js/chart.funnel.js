...
function labelAbbreviation(label, characterLimit) {
    var result = label;
...
module.exports = function (Chart) {
	...
	Chart.defaults.funnel = {
		...
		sort: 'desc', // sort options: 'asc', 'desc'
		gap: 0,
				...
				generateLabels: function (chart) {
					...
					if (data.labels.length && data.datasets.length) {
						return data.labels.map(function (label, i) {
						    label = labelAbbreviation(label, 10);
	...
	Chart.controllers.funnel = Chart.DatasetController.extend({
		...
		initialize: function (chart, datasetIndex) {
			...
			if (typeof chart.options !== 'undefined' && typeof chart.options.sort !== 'undefined' && chart.options.sort.substr(0, 4) !== 'data') {
				...
				// sort domains in the same manner as data sort order
				var domain =  chart.data.domains;
				if (domain) {
                    var domains = chart.data.domains.map((value, index) => {
                        return chart.data.domains[ dataPositions[index].index ];
                    });
                    chart.data.domains = domains;
				}
		...
		updateElement: function updateElement(trapezium, index, reset) {
			...
			var total_index = me.valAndLabels.length;
            var max_width = chart.width * 0.5;
			...
			if (sort === 'asc' || sort === 'data-asc' || !sort) {
                ...
                upperWidth = (index * gradient) / total_index * max_width || me.topWidth;
                bottomWidth = (index * gradient + gradient) / total_index * max_width;
				...
				upperWidth = (total_index - (index * gradient) + gradient) / total_index  * max_width;
				bottomWidth = (total_index - (index * gradient)) / total_index * max_width || me.topWidth;
			}
...
module.exports = function (Chart) {
	...
	Chart.elements.Trapezium = Chart.elements.Rectangle.extend({
		...
		draw: function () {
			...
			var vm = this._view;
			var globalOptionTrapeziumElements = globalOpts.elements.trapezium;
			var labels = this._chart.config.data.labels;
			var values = this._chart.config.data.datasets[0].data;
			var labelUnit = this._chart.options.labelUnit || '';
			...
			var center_x = (cornerAt(2)[0] + cornerAt(1)[0]) / 2;
			var center_y = (cornerAt(0)[1] + cornerAt(1)[1]) / 2;
			var fontStyle = this._chart.options.defaultFontFamily;
			var fontSize = 15;
			ctx.beginPath();
			...
			var currentValue = numFormatter(values[vm.index], 4) + " " + labelUnit ;
			ctx.fillText(currentValue, center_x, center_y);
