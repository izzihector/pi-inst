...
var StyleBarController = BarController.extend({
	...
	calculateBarValuePixels: function(datasetIndex, index) {
		...
		if (minBarLength !== undefined && Math.abs(size) < minBarLength && Math.abs(size) !== 0) {
			size = minBarLength;
...
var StyleDoughnutController = DoughnutController.extend({
	...
	draw: function(element) {
		...
		DoughnutController.prototype.draw.apply(me, arguments);

        if (me.chart.options.centerText){
            // draw the center text of doughnut data sum
            var fontSize = 30;
            var fontStyle = me.chart.options.defaultFontFamily;
            var ctx = me.chart.ctx;
            var chartArea = me.chart.chartArea;
            var centerX = (chartArea.left + chartArea.right) / 2;
            var centerY = (chartArea.top + chartArea.bottom) / 2;
            var total = numFormatter(me.calculateTotal(), 4);
...
var GaugeController = Chart.controllers.tsgauge;

var StyleGaugeController = GaugeController.extend({
	...
	updateElement: function(arc, index) {
		...
		GaugeController.prototype.updateElement.apply(me, arguments);
	...
	setHoverStyle: function(element) {
		...
		GaugeController.prototype.setHoverStyle.apply(me, arguments);
	...
	removeHoverStyle: function(element) {
		...
		GaugeController.prototype.removeHoverStyle.apply(me, arguments);
	}
});

var FunnelController = Chart.controllers.funnel;
...
defaults$1.global.legend.labels.generateLabels = function(chart) {
	...
	return helpers$b.isArray(data.datasets) ? data.datasets.map(function(dataset, i) {
	    var label = labelAbbreviation(dataset.label, 10);
